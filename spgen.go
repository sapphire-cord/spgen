...
func parseComments(cms *ast.CommentGroup) (description string, usage string, aliases []string, disabled bool, cooldown int, guildonly bool) {
	...
	for _, line := range split {
		...
		if strings.HasPrefix(lower, "usage:") {
			...
			splitinp := line[8:]
			if strings.HasPrefix(splitinp, " ") {
				splitinp = splitinp[1:]
			}
			split := strings.Split(splitinp, ",")
			for _, alias := range split {
					...
					if star, ok := fn.Type.Params.List[0].Type.(*ast.StarExpr); ok { // contexts are passed by pointer, therefore StarExpr
						...
						if sel, ok := star.X.(*ast.SelectorExpr); ok {
							...
							if pkg, ok := sel.X.(*ast.Ident); ok {
								...
								if *verbose {
									fmt.Printf("Found sapphire command: %s\n", name)
								}
...
`, strings.Join(imports, "\n  "))
	for _, cmd := range commands {
		...
		if cmd.Description != "" {
			src += fmt.Sprintf(".SetDescription(\"%s\")", strings.Replace(cmd.Description, "\"", "\\\"", -1))
		}
		if cmd.OwnerOnly {
			src += ".SetOwnerOnly(true)"
		}
		if cmd.Usage != "" {
			src += fmt.Sprintf(".SetUsage(\"%s\")", strings.Replace(cmd.Usage, "\"", "\\\"", -1))
		}
		if cmd.Disabled {
			src += ".Disable()"
		}
		if cmd.Cooldown > 0 {
			src += fmt.Sprintf(".SetCooldown(%d)", cmd.Cooldown)
		}
		if cmd.GuildOnly {
			src += ".SetGuildOnly(true)"
		}
		if len(cmd.Aliases) > 0 {
			aliases := cmd.Aliases
			for i, al := range aliases {
				aliases[i] = "\"" + strings.Replace(al, "\"", "\\\"", -1) + "\""
			}
